...
var HTMLFILE_URL = "googleoutput.html";
...
var buildfn = function() {
    var response2file = function(result, response) {
        if (result instanceof Error) {
            ...
            console.error("Wrote %s", HTMLFILE_URL);
            fs.writeFileSync(HTMLFILE_URL, result);
        }
    };
...
if(require.main == module) {
    program
        ...
        .option('-f, --url <url>', 'Url to retreive the content', URL_DEFAULT)
        .parse(process.argv);
    readUrl(program.url); 
    var checkJson = checkHtmlFile(HTMLFILE_URL, program.checks);
